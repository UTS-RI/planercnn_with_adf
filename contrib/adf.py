...
class ReLU(nn.Module):
    def __init__(self,inplace, keep_variance_fn=None):
...
class MaxPool2d(_MaxPoolNd):
        ...
        def __init__(self, kernel_size, stride=None, padding=0, dilation=1,
                     return_indices=False, ceil_mode=False,keep_variance_fn=None):
        ...
        def forward(self, input,input_variance):
            if self._keep_variance_fn is not None:
                output_var = F.max_pool2d(input_variance, self.kernel_size, self.stride, self.padding, self.dilation, self.ceil_mode,self.return_indices)
                outputs_variance = self._keep_variance_fn(output_var)
                return F.max_pool2d(input, self.kernel_size, self.stride,self.padding, self.dilation, self.ceil_mode,self.return_indices),outputs_variance
            else:
                return F.max_pool2d(input, self.kernel_size, self.stride, self.padding, self.dilation, self.ceil_mode,self.return_indices), F.max_pool2d(input_variance, self.kernel_size, self.stride, self.padding, self.dilation, self.ceil_mode,self.return_indices)
#
...
#
#         if inputs_mean.size()[3]%2 ==0:
#             mu_a = inputs_mean[:, :, :, 0::2]
#             mu_b = inputs_mean[:, :, :, 1::2]
#             var_a = inputs_variance[:, :, :, 0::2]
#             var_b = inputs_variance[:, :, :, 1::2]
#         else:
#             inputs_mean_fix =inputs_mean[:,:,:,:-1]
#             inputs_variance_fix=inputs_variance[:,:,:,:-1]
#             mu_a = inputs_mean_fix[:, :, :, 0::2]
#             mu_b = inputs_mean_fix[:, :, :, 1::2]
#             var_a = inputs_variance_fix[:, :, :, 0::2]
#             var_b = inputs_variance_fix[:, :, :, 1::2]
#
...
class Sequential(nn.Module):
    ...
    def forward(self, inputs, inputs_variance):
        ...
        for module in self._modules.values():
            ...
            # print(inputs_variance)
            # print(inputs)
            #inputs_variance = module(inputs_variance)
...
class SamePad2d(nn.Module):
    ...
    def forward(self, input,input_variance):
        ...
        return F.pad(input, (pad_left, pad_right, pad_top, pad_bottom), 'constant', 0) ,F.pad(input_variance, (pad_left, pad_right, pad_top, pad_bottom), 'constant', 0)
...
class Sigmoid(nn.Module):
    ...
    def forward(self, features_mean, features_variance, eps=1e-5):
        ...
        log_gaussian_mean = torch.exp(log_gaussian_mean) / (1 + torch.exp(log_gaussian_mean))
        log_gaussian_variance = torch.exp(log_gaussian_variance) / (1 + torch.exp(log_gaussian_variance))
        log_gaussian_variance = log_gaussian_variance * (torch.exp(features_variance) - 1)

        # constant = torch.sum(log_gaussian_mean, dim=self.dim) + eps
        # constant = constant.unsqueeze(self.dim)
        outputs_mean = log_gaussian_mean
        outputs_variance = log_gaussian_variance
...
class Upsample(nn.Module):
    ...
    def forward(self, input, input_variance):
        ...
        return F.interpolate(input, self.size, self.scale_factor, self.mode, self.align_corners), F.interpolate(input_variance, self.size, self.scale_factor, self.mode, self.align_corners)
