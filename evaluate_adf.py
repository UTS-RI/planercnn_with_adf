...
from models_adf.model import *
from models_adf.refinement_net import RefineModel
...
class PlaneRCNNDetector():
    ...
    def detect(self, sample):
        ...
        for indexOffset in [0, ]:
            ...
            planes, gt_segmentation = sample[indexOffset + 0].cuda(), sample[indexOffset + 1].numpy(),\
            ...
            target_mask, mrcnn_mask, target_parameters, mrcnn_parameters, detections, detection_masks_mean,detection_masks_var, \
            detection_gt_parameters, detection_gt_masks, rpn_rois, roi_features, roi_indices, depth_np_pred \
                = self.model.predict([images, image_metas, gt_class_ids, gt_boxes, gt_masks, gt_parameters, camera],
            ...
            if len(detections) > 0:
                ...
                detections, detection_masks = unmoldDetections(self.config, camera, detections, detection_masks_mean,detection_masks_var, depth_np_pred, debug=False)
                pass
            # at this point detection masks is detection_masks_mean
            XYZ_pred, detection_mask, plane_XYZ = calcXYZModule(self.config, camera, detections, detection_masks,detection_masks_var, depth_np_pred, return_individual=True)
        ...
        if ('refine' in self.modelType or 'refine' in self.options.suffix):
            ...
            for c in range(1):
                ...
                depth_np = torch.nn.functional.interpolate(depth_np[0][:, 80:560].unsqueeze(1), size=(192, 256), mode='bilinear').squeeze(1)
        ...
        for sampleIndex, sample in enumerate(data_iterator):
            ...
            if 'inference' not in options.dataset:
                ...
                for c in range(len(detection_pair)):
                    ...
                    np.save(options.test_dir + '/' + str(sampleIndex % 500) + '_plane_parameters_variance' + str(c) + '.npy', detection_pair[c]['detection'][:,  9:12])
