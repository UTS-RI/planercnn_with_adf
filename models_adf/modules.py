...
def unmoldDetections(config, camera, detections, detection_masks,detection_masks_var, depth_np, unmold_masks=True, debug=False):
    ...
    if 'normal' in config.ANCHOR_TYPE:
        ...
        if config.NUM_PARAMETER_CHANNELS == 4:
            ...
            if config.FITTING_TYPE % 2 == 1:
                ...
                plane_parameter_uncertainitiy = detections[:, 9:12]     #uncertainity from mrcnn_parameter uncertainity

                plane_normals = plane_parameters / torch.clamp(torch.norm(plane_parameters, dim=-1, keepdim=True), 1e-4)
                plane_param_uncert_normal_temp = plane_parameter_uncertainitiy/ torch.clamp(torch.norm(plane_parameters, dim=-1, keepdim=True), 1e-4)
                ...
                plane_parameters = plane_normals * offsets.view((-1, 1))
                plane_parameter_uncertainity =  plane_param_uncert_normal_temp* offsets.view((-1, 1))
                pass
            pass
        detections = torch.cat([detections[:, :6], plane_parameters,plane_parameter_uncertainity], dim=-1)
        # print(detections)
...
def calcXYZModule(config, camera, detections, masks,detection_masks_var, depth_np, return_individual=False, debug_type=0):
